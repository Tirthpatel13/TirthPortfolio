...
const NavBar = () => {
  ...
  return (
    <div className='flex justify-between items-center 
        ...
        <div>
            <h1 className='text-5xl font-signature ml-2'>Tirth</h1>
        </div>
        ...
        text-white-500 md:hidden'>
            {nav ? <FaTimes size={30} /> : <FaBars size={30} />}
        </div>

        {nav && (
        <ul className='flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-900 text-white-500'>
             {links.map((link) => (
                <li 
                key={link.id} 
